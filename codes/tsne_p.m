function ydata = tsne_p(P, labels, no_dims,max_iter,show_tsne)
...
while iter<max_iter
    ...
    if iter>100
        err_delta=cost_iter(end-5:end-1)-cost;
        mean(abs(err_delta));
        if mean(abs(err_delta))/cost<1e-5
            break
        end
    end
    % Display scatter plot (maximally first three dimensions)
    if ~rem(iter, 10) && ~isempty(labels) && show_tsne
